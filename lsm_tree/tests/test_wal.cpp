...
bool test_single_entry() {
    ...
    
    {
        ...
        entries = generate_test_entries(0, 1);
        write_entries_to_wal(wal, entries);

        cout << "WAL file location: " << wal.get_wal_file_location() << endl;
        cout << "Absolute path: " << fs::absolute(wal.get_wal_file_location()) << endl;
...
bool test_missing_wal_file() {
    ...
    wal_location += "missing_test.log";

    fs::remove(wal_location);
    
    {
        Wal wal("missing_test.log", wal_location);
...
bool test_corrupted_wal_invalid_length() {
    ...
    string wal_location = DEFAULT_WAL_FOLDER_LOCATION;
    wal_location += "corrupted_test.log";
    ...
    {
        Wal wal("corrupted_test.log", wal_location);
...
bool test_partial_entry() {
    ...
    {
        Wal wal("partial_test.log", wal_location);

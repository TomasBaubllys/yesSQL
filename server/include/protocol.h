...
using protocol_id_type = uint64_t;
#define protocol_id_hton(x) htonl(x)
#define protocol_id_ntoh(x) ntohl(x)

#define PROTOCOL_COMMAND_NUMBER_POS (sizeof(protocol_id_type) + sizeof(protocol_message_len_type) + sizeof(protocol_array_len_type))
